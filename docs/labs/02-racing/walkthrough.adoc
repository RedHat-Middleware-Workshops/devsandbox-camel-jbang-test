...
[NOTE]
====
Click pass:[<span style="font-size: 35px;">↻</span>] to refresh your project file explorer, and make sure to unfold the boxes directory to see the *Team Camel* car.
====
...
For better watching we recommend you organize your terminals horizontally. If you are using VS Code or DevSpaces, you can do this by dragging the terminal window to the side.
...
As the event is underway, you'll see team radio messages appearing in your terminal.
...
The grandstands are full and the crowd is excited.
...
NOTE: The new route is added to the `broadcast.yaml` file opening a new interface to display commercials. 
...
Copy and paste the command below to inject a commercial:
...
The on-track battle is heating up and laps are flying by quickly. Drivers are running out of laps as the race is almost over.
...
Let's do a quick review of the commands you used in this lab:
