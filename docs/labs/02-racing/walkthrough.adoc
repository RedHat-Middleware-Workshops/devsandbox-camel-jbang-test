...
image::images/track-practice.png[align="left", width=15%]
...
Feel free to let the team go back on track to put more mileage under their belts. +
The more laps they cover the more data they can collect for analysis and improve their car.
...
When race control sends `practice-open` or `boxes`, the teams write messages such as "🟥 Time to hit the track, practice is open!" or "🟨 Please enter boxes at the end of the lap!". The `receive` command watches the `radio` directory and prints the body of each new message.
...
** Run a team with a livery: `camel run team.yaml --property=livery=🟥 --name team1`
